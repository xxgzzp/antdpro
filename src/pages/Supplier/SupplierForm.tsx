...
import { apiOaSupplierCreate, apiOaSupplierUpdate } from '@/services/ant-design-pro/api';
...
const SupplierForm: React.FC<{
  ...
  const handleFinish = async (formData: API.Supplier) => {
    if (typeAddOrUpdate) {
      await apiOaSupplierCreate(formData).then((data) => {
        // 新增Supplier时，set回去
        if (setSupplierList) {
      ...
      await apiOaSupplierUpdate(
        { id: updateInit?.id } as API.apiOaSupplierUpdateParams,
        formData,
      ).then(() => {
        // 关闭模态框
        setModalOpen(false);
