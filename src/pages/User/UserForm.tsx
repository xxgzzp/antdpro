...
const UserForm: React.FC<{
...
}> = ({modalOpen, setModalOpen, reload,updateUserInit,typeAddOrUpdate=true}) => {
  ...
  const handleFinish =async (formData: API.User) => {
    if (typeAddOrUpdate) {
      await apiOaUserCreate(formData).then(() => {
        // 关闭模态框
        setModalOpen(false)
        // 重新加载页面
        reload?.()
      ...
      await apiOaUserPartialUpdate({id: updateUserInit?.id} as API.apiOaUserUpdateParams,formData).then(() => {
        // 关闭模态框
        setModalOpen(false)
        // 重新加载页面
        reload?.()
  ...
  return (
    <>
      ...
      >
        <ProFormText width="md" name="mobile_phone" label="手机号" placeholder="请输入手机号"
          // @ts-ignore
